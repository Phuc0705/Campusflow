...
router.post('/optimize', async (req, res) => {
  try {
    ...
    const aiResponse = await axios.post(`${AI_SERVICE_URL}/api/optimize/check-conflict`, {
      events, duration_minutes: 60
    });
    const conflictData = aiResponse.data;

    const suggestResponse = await axios.post(`${AI_SERVICE_URL}/api/optimize/suggest-slots`, {
      events, duration_minutes: 60
    });
